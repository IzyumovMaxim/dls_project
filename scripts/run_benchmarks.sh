...
python scripts/bench/benchmark_compression.py --index-dir "$FLAT" --benchmark "$BENCH"
...
for cfg in ivf ivfpq hnsw; do
  ...
  python scripts/index/build_index.py --config "configs/e5_base_$cfg.yaml" --from-embeddings "$FLAT"
  python scripts/evaluate.py    --config "configs/e5_base_$cfg.yaml" --benchmark "$BENCH"
  python scripts/bench/latency.py     --config "configs/e5_base_$cfg.yaml" --index-dir "data/index/e5_base_$cfg"
done

# Flat latency + nprobe/efSearch sweep (recall<->latency curves).
python scripts/bench/latency.py    --config configs/e5_base_flat.yaml --index-dir "$FLAT"
echo "== axis B: sweep =="
python scripts/bench/sweep_index.py --benchmark "$BENCH"
