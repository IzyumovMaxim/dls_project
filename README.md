...
**Инференс**: достаточно CPU, GPU не требуется. Пиковое потребление памяти — около 2.1 ГБ RSS (включая модель кодирования и OPQ-индекс). Время обработки одного запроса (end-to-end latency): ~20 мс на кодирование + 37 мс на поиск + 3 мс на поиск предложения-доказательства (~89 мс на CPU).

**Сборка индекса**: кодирование 500 000 пассажей и 2.26 млн предложений, а также дообучение модели, практичны только на GPU.
