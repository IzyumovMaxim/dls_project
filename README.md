...
Таблицы находятся в `data/analysis/RESULTS.md`, сырые числа - в `benchmark_all.json`. Эти файлы генерируются через `scripts/bench/benchmark_all.py`.

Приложение обслуживает `configs/e5_base_opq192.yaml`. Выбрали эту конфигурацию после сравнения метрик:
...
- **Сжатие, а не отсечение.** IVF/HNSW разгоняют поиск до миллисекунд, но проседают по метрикам качества.Время не очень критично: запрос кодируется ~50 мс на CPU, и разница между 37 мс поиска и 1 мс для пользователя незаметна. Существенно различаются индексы по памяти.
- **m=192.** 96 суб-квантизаторов (настройка по умолчанию) стоят −0.048 nDCG; удвоение кода до 192 байт снижает потерю до −0.0087.
- **выбрали OPQ, а не PQ.** OPQ перевешивает (+0.0038 P@1) сильнее, чем разброс от случайной инициализации k-means при обучении кодбуков PQ (0.013).Единственная просадка — 8 минут разовой сборки. Подробности в `RESULTS.md`.
